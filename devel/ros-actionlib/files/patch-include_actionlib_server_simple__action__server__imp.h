...
@@ -63,8 +63,7 @@ SimpleActionServer<ActionSpec>::SimpleActionServer(std
...
+: new_goal_(false), preempt_request_(false), new_goal_preempt_request_(false), execute_thread_(nullptr), need_to_terminate_(false)
 {
   // create the action server
   as_ = boost::shared_ptr<ActionServer<ActionSpec> >(new ActionServer<ActionSpec>(n_, name,
@@ -117,7 +116,7 @@ template<class ActionSpec>
...
+  execute_thread_(nullptr), need_to_terminate_(false)
 {
   // create the action server
